...
export class WorkflowController {
  ...
  async startRun(@Body() body: { sopId: string; runId?: string }) {
    ...
    const existing = await this.prisma.client.workflowRun.findUnique({ where: { id: runId } });
    if (existing) {
    ...
    await this.runRepo.initializeRun(
      runId, body.sopId, validatedDag.start_node_id, JSON.stringify(initialSnapshot)
    );

    return { run_id: runId, status: 'ACTIVE', current_step: validatedDag.start_node_id };
  }
  ...
  async processEvent(@Param('id') id: string, @Body() eventPayload: { type: string; payload: any }) {
    const runMetadata = await this.prisma.client.workflowRun.findUnique({ where: { id } });
    ...
    const isSensorUpdate = eventPayload.type === 'YOLO_UPDATE' || eventPayload.type === 'WEIGHT_UPDATE';
    if (!isSensorUpdate) {
    ...
    return {
      ...
      deviation: deviationTriggered
        ? newSnapshot.context.deviations[newSnapshot.context.deviations.length - 1]
        : null
    };
  }
  ...
  async getEvents(@Param('id') id: string) {
    const run = await this.prisma.client.workflowRun.findUnique({ where: { id } });
  ...
  async getReport(@Param('id') id: string) {
    ...
    const stepRows = validatedDag.nodes.map(node => {
      const successEvent = events.find(
        e => e.eventType === 'EXECUTE_STEP' && e.validationResult === 'PASS'
          && snapshot.context.completedNodes.includes(node.id)
          // Match by order of completion
      );
      const deviations = snapshot.context.deviations.filter(d => d.stepId === node.id);
      ...
      return {
        ...
        title: node.title,
        type: node.type,
        status: completed ? 'PASS' : deviations.length > 0 ? 'DEVIATION' : 'PENDING',
        submitted_value: submittedValue,
        target_value: node.type === 'MEASUREMENT' ? node.config.target_value : null,
        unit: node.type === 'MEASUREMENT' ? node.config.unit : null,
        tolerance: node.type === 'MEASUREMENT'
          ? `±${node.config.tolerance_positive}${node.config.unit}` : null,
        expected_entity: node.type === 'VERIFICATION' ? node.config.entity_name : null,
    ...
    return {
      ...
      pending_steps: validatedDag.nodes.length - snapshot.context.completedNodes.length,
      operator_summary: {
